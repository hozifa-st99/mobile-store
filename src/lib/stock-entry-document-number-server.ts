...
import {
  formatStockEntryDocumentNumber,
  parseStockEntryDocumentSeq,
} from "@/lib/stock-entry-document-number";
...
export async function getMaxStockEntryDocumentSeq(
  db: Db,
  branchId: string,
  branchCode?: string
): Promise<number> {
  ...
  for (const row of rows) {
    const seq = parseStockEntryDocumentSeq(row.documentNumber, code);
    if (seq != null && seq > maxSeq) maxSeq = seq;
  }
  return maxSeq;
}

export async function getNextStockEntryDocumentNumber(
  db: Db,
  branchId: string
): Promise<string> {
  const branchCode = await getBranchCode(db, branchId);
  const maxSeq = await getMaxStockEntryDocumentSeq(db, branchId, branchCode);
  return formatStockEntryDocumentNumber(branchCode, maxSeq + 1);
}

export async function allocateStockEntryDocumentNumber(
  db: Db,
  branchId: string,
  requested?: string
): Promise<string> {
  ...
  if (trimmed) {
    const exists = await db.stockEntry.findFirst({
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    const candidate = await getNextStockEntryDocumentNumber(db, branchId);
    const exists = await db.stockEntry.findFirst({
